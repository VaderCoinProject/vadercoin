...
getNameSalt(const CKey& key, const valtype& name, valtype& rand)
{
    ...
    const std::string info("Vadercoin Registration Salt");
      ...
      [&] (const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
  ...
  if (chainman.ActiveChainstate ().IsInitialBlockDownload ())
    throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD,
                       "Vadercoin is downloading blocks...");
  ...
  /* sendtoname does not support an options argument (e.g. to override the
     ...
     https://github.com/vadercoin/vadercoin-core/issues/12.  */
