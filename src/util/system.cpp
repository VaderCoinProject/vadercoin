...
// For details see https://github.com/vadercoin/vadercoin/pull/22348.
...
const char * const VADERCOIN_CONF_FILENAME = "vadercoin.conf";
const char * const VADERCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/vadercoin/vadercoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //vadercoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", VADERCOIN_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "vadercoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Vadercoin
    // macOS: ~/Library/Application Support/Vadercoin
    // Unix-like: ~/.vadercoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Vadercoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Vadercoin";
#else
    // Unix-like
    return pathRet / ".vadercoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", VADERCOIN_CONF_FILENAME);
