...
#include <qt/vadercoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Vadercoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new VadercoinAddressEntryValidator(parent));
    widget->setCheckValidator(new VadercoinAddressCheckValidator(parent));
}
...
bool parseVadercoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no vadercoin: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!VadercoinUnits::parse(VadercoinUnits::VADE, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseVadercoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseVadercoinURI(uriInstance, out);
}

QString formatVadercoinURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(VadercoinUnits::format(VadercoinUnits::VADE, info.amount, false, VadercoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openVadercoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", VADERCOIN_CONF_FILENAME));
    ...
    /* Open vadercoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Vadercoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Vadercoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Vadercoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Vadercoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "vadercoin.desktop";
    return GetAutostartDir() / strprintf("vadercoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a vadercoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Vadercoin\n";
        else
            optionFile << strprintf("Name=Vadercoin (%s)\n", chain);
