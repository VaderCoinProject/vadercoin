    ...
    : QWidget(parent), m_platform_style{platformStyle}
{
    ...
    // Scaled by 82 to 97 in Vadercoin due to "Name operation" width as rendered
    // on Whonix 15 KDE.
    if (platformStyle->getUseExtraSpacing()) {
...
void TransactionView::changedAmount()
{
    ...
    if (VadercoinUnits::parse(model->getOptionsModel()->getDisplayUnit(), amountWidget->text(), &amount_parsed)) {
...
void TransactionView::exportClicked()
{
    ...
    writer.addColumn(VadercoinUnits::getAmountColumnTitle(model->getOptionsModel()->getDisplayUnit()), 0, TransactionTableModel::FormattedAmountRole);
