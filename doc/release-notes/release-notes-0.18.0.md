Vadercoin Core version 0.18.0 is now available from:

  <https://vadercoincore.org/bin/vadercoin-core-0.18.0/>
...
`/Applications/Vadercoin-Qt` (on Mac) or `vadercoind`/`vadercoin-qt` (on
Linux).
...
recommended to use Vadercoin Core on unsupported systems.
...
10.10. Additionally, Vadercoin Core does not yet change appearance when
...
`contrib/init/vadercoind.service`, it has been changed to use
`/var/lib/vadercoind` as the data directory instead of
`~vadercoin/.vadercoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/vadercoind` will
exist has enough space (check using `df -h /var/lib/vadercoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/vadercoin/vadercoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://vadercoincore.org/en/contact/) the Vadercoin Core
...
The systemd init file (`contrib/init/vadercoind.service`) has been changed
to use `/var/lib/vadercoind` as the data directory instead of
`~vadercoin/.vadercoin`. This change makes Vadercoin Core more consistent with
...
`datadir` option in `/etc/vadercoin/vadercoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/vadercoin/vadercoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/vadercoin/vadercoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/vadercoin/vadercoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/vadercoin/vadercoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/vadercoin/vadercoin/blob/master/doc/vadercoin-conf.md)
  about the `vadercoin.conf` file describes how to use it to configure
  Vadercoin Core.

- A new document introduces Vadercoin Core's BIP174 [Partially-Signed
  Vadercoin Transactions
  (PSBT)](https://github.com/vadercoin/vadercoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/vadercoin/vadercoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Vadercoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Vadercoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/vadercoin/vadercoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/vadercoin/vadercoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Vadercoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Vadercoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/vadercoin/vadercoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/vadercoin/vadercoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart vadercoind with the `-deprecatedrpc=generate`
...
- A new `vadercoin-wallet` tool is now distributed alongside Vadercoin
...
This section describes planned changes to Vadercoin Core that may affect
other Vadercoin software and services.

- Since version 0.16.0, Vadercoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Vadercoin Core 0.20 (expected about
  a year after 0.18), Vadercoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Vadercoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Vadercoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Vadercoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Vadercoin Core
...
- A sub-project of Vadercoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Vadercoin Core.  See their [project
  page](https://github.com/vadercoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Vadercoin Core's own implementation, although entropy
  gathered by Vadercoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Vadercoin Core a
...
- On macOS, Vadercoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `vadercoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libvadercoin_server.a` (jnewbery)
...
- #13926 [Tools] vadercoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like vadercoind (MarcoFalke)
...
- #14521 qt, docs: Fix `vadercoin-qt -version` output formatting (hebasto)
...
- #11625 Add VadercoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libvadercoin_server` (laanwj)
...
- #15154 Configure: vadercoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/vadercoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Vadercoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libvadercoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in vadercoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Vadercoin Core (Christewart)
...
- #12246 Bugfix: Only run vadercoin-tx tests when vadercoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_vadercoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_vadercoin` (MarcoFalke)
...
- #14186 vadercoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_vadercoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update vadercoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/vadercoin-conf.md (hebasto)
...
- #15354 Add missing `vadercoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct vadercoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support vadercoin folder on OSX (JimmyMow)
