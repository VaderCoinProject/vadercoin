Vadercoin Core version 0.11.0 is now available from:

  <https://vadercoin.org/bin/vadercoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer VADE/kB fee to be
...
For example, add the following to `vadercoin.conf`:
...
related to the blockchain in the vadercoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Vadercoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Vadercoin Core will 
...
For an example script see [vadercoin-submittx](https://github.com/laanwj/vadercoin-submittx).
...
- #5334 `cf87536` libvadercoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new vadercoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only VadercoinAmountField
- #5753 `0f44672` Add vadercoin logo to about screen
...
- #5809 `46bfbe7` Add vadercoin-cli man page
...
- #5849 `41259ca` contrib/init/vadercoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Vadercoin-Qt.app to Vadercoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for vadercoin-tx)
