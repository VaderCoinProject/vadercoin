...
The Vadercoin Core daemon can be configured to act as a trusted "border
router", implementing the vadercoin wire protocol and relay, making
...
The ZeroMQ feature in Vadercoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building vadercoind:
...
For instance:
    ...
    $ vadercoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/vadercoind.tx.raw \
...
These options can also be provided in vadercoin.conf.
...
From the perspective of vadercoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into vadercoind directly. Furthermore, no information is
...
using. Vadercoind appends an up-counting sequence number to each
