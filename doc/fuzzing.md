# Fuzzing Vadercoin Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Vadercoin Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/vadercoin/vadercoin
$ cd vadercoin/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/vadercoin/vadercoin/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/vadercoin/vadercoin/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/vadercoin/vadercoin/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/vadercoin/vadercoin/tree/master/src/test/fuzz).
...
The project's collection of seed corpora is found in the [`vadercoin-core/qa-assets`](https://github.com/vadercoin-core/qa-assets) repo.

To fuzz `process_message` using the [`vadercoin-core/qa-assets`](https://github.com/vadercoin-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/vadercoin-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`vadercoin-core/qa-assets`](https://github.com/vadercoin-core/qa-assets) repo.

Every single pull request submitted against the Vadercoin Core repo is automatically tested against all inputs in the [`vadercoin-core/qa-assets`](https://github.com/vadercoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Vadercoin Core more robust.
...
with certain assembly code from Vadercoin Core's code. See [developer notes on sanitizers](https://github.com/vadercoin/vadercoin/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Vadercoin Core using afl++

## Quickstart guide

To quickly get started fuzzing Vadercoin Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Vadercoin Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Vadercoin Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Vadercoin Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Vadercoin
Core without having to write any custom fuzzing harness. The `vadercoind` server
...
```sh
$ mkdir vadercoin-honggfuzz-p2p/
$ cd vadercoin-honggfuzz-p2p/
$ git clone https://github.com/vadercoin/vadercoin
$ cd vadercoin/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/vadercoind.cpp b/src/vadercoind.cpp
index 455a82e39..2faa3f80f 100644
--- a/src/vadercoind.cpp
+++ b/src/vadercoind.cpp
...
Vadercoin Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/vadercoin-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=vadercoin-core).
...
because Vadercoin is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_vadercoin-core/latest).
