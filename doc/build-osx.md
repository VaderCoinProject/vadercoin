...
This guide describes how to build vadercoind, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Vadercoin Core from source.
...
### 4. Clone Vadercoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Vadercoin Core repository to a directory.
...
It is not necessary to build wallet functionality to run `vadercoind` or  `vadercoin-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/vadercoin/vadercoin-core/blob/master/doc/descriptors.md), `sqlite` is required.
...
Vadercoin Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/vadercoin/vadercoin/issues/7714).
...
You can deploy a `.dmg` containing the Vadercoin Core application using `make deploy`.
...
## Build Vadercoin Core

1. Clone the Vadercoin Core source code:
    ```shell
    git clone https://github.com/vadercoin/vadercoin-core
    cd vadercoin-core
    ```

2.  Build Vadercoin Core:

    Configure and build the headless Vadercoin Core binaries as well as the GUI (if Qt is found).
...
Run the following in your terminal to compile Vadercoin Core:
...
## Running Vadercoin Core

Vadercoin Core should now be available at `./src/vadercoind`.
If you compiled support for the GUI, it should be available at `./src/qt/vadercoin-qt`.

The first time you run `vadercoind` or `vadercoin-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Vadercoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Vadercoin"

touch "/Users/${USER}/Library/Application Support/Vadercoin/vadercoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Vadercoin/vadercoin.conf"
```

The first time you run vadercoind, it will start downloading the blockchain. This process could
take several hours.

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Vadercoin/debug.log
```

## Other commands:

```shell
./src/vadercoind -daemon      # Starts the vadercoin daemon.
./src/vadercoin-cli --help    # Outputs a list of command-line options.
./src/vadercoin-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/vadercoin-qt -server # Starts the vadercoin-qt server mode, allows vadercoin-cli control
```
