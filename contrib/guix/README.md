# Bootstrappable Vadercoin Core Builds

This directory contains the files necessary to perform bootstrappable Vadercoin
Core builds.
...
To build Vadercoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Vadercoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/vadercoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/vadercoin/guix.sigs).
...
This is especially helpful if you build both Vadercoin Core and Vadercoin Core,
since it will let Vadercoin and Vadercoin share the caches and SDKs.
