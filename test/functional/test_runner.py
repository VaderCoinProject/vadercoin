...
`test/functional/test_framework/test_framework.py:VadercoinTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    # FIXME: Reenable with data created for Vadercoin.
...
def main():
    ...
    enable_vadercoind = config["components"].getboolean("ENABLE_VADERCOIND")

    if not enable_vadercoind:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    # Warn if vadercoind is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "vadercoind"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a vadercoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
...
class RPCCoverage():
    ...
    commands per `vadercoin-cli help` (`rpc_interface.txt`).
